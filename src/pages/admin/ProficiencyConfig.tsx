      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight flex items-center">
             ...
             Cấu hình Test Năng lực
          </h1>
          <p className="text-gray-500 font-medium">Thiết lập logic AI và thuật toán phân nhánh cho bài kiểm tra trình độ.</p>
        </div>
        ...
        <Button onClick={() => navigate('/admin/proficiency-config/new')} className="chinese-gradient h-11 px-6 rounded-xl font-black text-white shadow-lg shadow-primary/20 transition-all hover:-translate-y-1">
          <Plus className="mr-2 h-4 w-4" /> Tạo cấu hình mới
        </Button>
      </div>
      ...
      {configs.length === 0 ? (
        <div className="bg-white p-20 rounded-[3rem] border border-gray-100 shadow-sm text-center space-y-6">
           ...
           <div className="space-y-2">
              <h3 className="text-2xl font-black text-gray-900">Chưa có kịch bản test</h3>
              <p className="text-gray-500 font-medium">Hãy khởi tạo kịch bản đầu tiên để học viên có thể đánh giá năng lực.</p>
           </div>
           <Button onClick={() => navigate('/admin/proficiency-config/new')} className="chinese-gradient h-12 px-8 rounded-xl font-black text-white shadow-lg">Bắt đầu thiết lập</Button>
        </div>
      ) : (
        <div className="grid gap-8">
          {configs.map((config) => (
            <div key={config._id} className="bg-white rounded-[3rem] p-8 md:p-10 border border-gray-100 shadow-xl relative overflow-hidden group">
               ...
               <div className="relative z-10 space-y-8">
                  <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                     ...
                     <div className="flex items-center space-x-2">
                        ...
                        <Button variant="ghost" size="icon" onClick={() => navigate(`/admin/proficiency-config/${config._id}/edit`)} className="w-10 h-10 rounded-xl hover:bg-blue-50 hover:text-blue-600"><Edit className="w-4 h-4" /></Button>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                     <div className="bg-gray-50 p-6 rounded-[2rem] border border-gray-50 text-center space-y-1 group-hover:bg-white group-hover:shadow-md transition-all">
                        <p className="text-[9px] font-black uppercase text-gray-400 tracking-widest">Lệ phí Xu</p>
                     ...
                     <div className="bg-gray-50 p-6 rounded-[2rem] border border-gray-50 text-center space-y-1 group-hover:bg-white group-hover:shadow-md transition-all">
                        <p className="text-[9px] font-black uppercase text-gray-400 tracking-widest">Câu hỏi Initial</p>
                        <p className="text-xl font-black text-gray-900">{config.initialQuestions.length} Levels</p>
                     </div>
                     <div className="bg-gray-50 p-6 rounded-[2rem] border border-gray-50 text-center space-y-1 group-hover:bg-white group-hover:shadow-md transition-all">
                        <p className="text-[9px] font-black uppercase text-gray-400 tracking-widest">Nhánh xử lý</p>
                        <p className="text-xl font-black text-gray-900">{config.branches.length} Branches</p>
                     </div>
                     <div className="bg-gray-50 p-6 rounded-[2rem] border border-gray-50 text-center space-y-1 group-hover:bg-white group-hover:shadow-md transition-all">
